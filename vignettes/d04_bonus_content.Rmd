...
`r Biocpkg("iSEEde")` and `r Biocpkg("iSEEpathways")` and ideal companion packages for exploring differential expression results in (e.g.) bulk RNA-seq data.
...
The processed dataset is provided with the workshop package.
...
As we previously learned, we can do a lot of these tasks by running the command:
...
Then, add/remove the panels to our need. 
This can involve multiple steps (selecting the gene of interest, color by a specific `colData`...), or probably even write multiple lines of code.
